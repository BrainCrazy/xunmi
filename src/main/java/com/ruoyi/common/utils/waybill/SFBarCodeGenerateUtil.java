...
public class SFBarCodeGenerateUtil {
     ...
     * 条码是图片格式（JPG、PNG、GIF）
     *
     * @param generateImageBytes 生成条形码路径及名称
     * @param width              条码图片宽度
     * @param height             条码图片高度
     * @param picTyp             图片格式<br/>
     *                           JPG、PNG、GIF三种图片类型选择
     * @return boolean类型值
     * true 生成成功，false 生成失败
     */
    public static byte[] generateBarCode(byte[] generateImageBytes, int width,
        ...
        try {
            //读取图片
            bis = new ByteArrayInputStream(generateImageBytes);
            //转换成图片对象
            ...
            //保存图片
            ImageIO.write(tag, picTyp, bos);
            bos.flush();
            log.info("条码生成成功. 像素：" + width + "*" + height);
