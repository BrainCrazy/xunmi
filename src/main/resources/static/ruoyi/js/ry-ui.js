...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            // 确认信息
            confirm: function (id) {
                $.modal.confirm($.table._option.modalDesc, function () {
                    var url = $.common.isEmpty(id) ? $.table._option.confirmUrl : $.table._option.confirmUrl.replace("{id}", id);
                    if ($.table._option.type == table_type.bootstrapTreeTable) {
                        $.operate.get(url);
                    } else {
                        var data = {"ids": id};
            ...
            // 确认信息
            confirmStock: function (id) {
                $.modal.confirm("确认货物已入(出)库？", function () {
                    var url = $.common.isEmpty(id) ? $.table._option.confirmStockUrl : $.table._option.confirmStockUrl.replace("{id}", id);
                    if ($.table._option.type == table_type.bootstrapTreeTable) {
                        $.operate.get(url);
                    } else {
                        var data = {"ids": id};
            ...
            // 确认信息
            confirmOutStock: function (id) {
                $.modal.confirm("确认已发货？", function () {
                    var url = $.common.isEmpty(id) ? $.table._option.OutStockUrl : $.table._option.OutStockUrl.replace("{id}", id);
                    if ($.table._option.type == table_type.bootstrapTreeTable) {
                        $.operate.get(url);
                    } else {
                        var data = {"ids": id};
            ...
            confirmOutStockTab: function (id) {
                $.modal.open("确认发货", $.operate.outStockUrl(id));
            },
            remove1: function (id) {
                $.modal.confirm("确定完成该条" + $.table._option.modalName + "信息吗？", function () {
                    var url = $.common.isEmpty(id) ? $.table._option.removeUrl1 : $.table._option.removeUrl1.replace("{id}", id);
                    if ($.table._option.type == table_type.bootstrapTreeTable) {
                        $.operate.get(url);
                    } else {
                        var data = {"ids": id};
            ...
            removeAll1: function () {
                ...
                $.modal.confirm("确认要完成选中的" + rows.length + "条数据吗?", function () {
                    var url = $.table._option.removeUrl1;
            ...
            outStockUrl: function (id) {
                var url = $.common.isEmpty(id) ? $.table._option.OutStockUrl.replace("{id}", "") : $.table._option.OutStockUrl.replace("{id}", id);
                return url;
            },
            // 修改信息
            edit: function (id) {
            ...
            moreEdit: function (id, url, moduleName) {
                if ($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    var row = $.bttTable.bootstrapTreeTable('getSelections')[0];
                    if ($.common.isEmpty(row)) {
                        $.modal.alertWarning("请至少选择一条记录");
                    ...
                    var turl = url.replace("{id}", row[$.table._option.uniqueId]);
                    $.modal.open(moduleName, turl);
                } else {
                    $.modal.open(moduleName, $.operate.editUrl1(id, url));
                }
            },
            userConfirm: function (id) {
                $.modal.open("", $.operate.userConfirmUrl(id));
            },
            // 修改信息
            addPackageNo: function (id) {
                if ($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    var row = $.bttTable.bootstrapTreeTable('getSelections')[0];
                    if ($.common.isEmpty(row)) {
                        $.modal.alertWarning("请至少选择一条记录");
                    ...
                    var url = $.table._option.addPackageUrl.replace("{id}", row[$.table._option.uniqueId]);
                    $.modal.open("添加运单号", url);
                } else {
                    $.modal.open("添加运单号", $.operate.addPackageNoUrl(id));
                }
            },
            addPackageNoUrl: function (id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.addPackageNoUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
            ...
            updateItem: function (id) {
                if ($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    ...
                    $.operate.submit(prefix + "/updateItem", "post", "json", data);
                }
            },
            // 修改信息，以tab页展现
            editTab: function (id) {
            ...
            userConfirmUrl: function (id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.userConfirmUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
